...
6. **Self-hosted Prometheus + Grafana on Railway** — Dockerfiles in `monitoring/prometheus/` and `monitoring/grafana/`. Prometheus scrapes `backend.railway.internal:8000/metrics` every 30s with 30d retention. Grafana auto-provisions the Prometheus data source and the STS dashboard. Both communicate over Railway's private network (IPv6, `fd12::` prefix). Only Grafana gets a public domain. See [[prometheus]] and [[grafana]].
